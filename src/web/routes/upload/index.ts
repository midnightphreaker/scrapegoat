...
import { UploadStagingService, ArchiveExtractor } from "../../upload/index";
...
export async function registerUploadRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/web/upload/start", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post("/web/upload/files", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const stagedFiles = [];
    const errors = [];
    const extractor = new ArchiveExtractor(DEFAULT_UPLOAD_CONFIG.maxArchiveEntries, DEFAULT_UPLOAD_CONFIG.maxArchiveUncompressedBytes);
      ...
      if (part.type === "file") {
        ...
        try {
          if (extractor.isArchiveBuffer(buffer)) {
            ...
            for (const e of result.errors) { errors.push(e); service.recordFailedFile(sessionId, { originalName: e.path, relativePath: e.path, error: e.error }); }
            for (const extracted of result.files) {
              const content = await fs.readFile(`${extractDir}/${extracted.relativePath}`);
              const staged = await service.stageFile(sessionId, extracted.relativePath, content, true, fileName);
              stagedFiles.push({ id: staged.id, name: staged.displayName, path: staged.relativePath, size: staged.size });
              await fs.unlink(`${extractDir}/${extracted.relativePath}`).catch(() => {});
            }
          ...
          service.recordFailedFile(sessionId, { originalName: fileName, relativePath: fileName, error: errorMsg });
        }
      }
    }
  ...
  server.get("/web/upload/tree", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const service = getStagingService();
    if (!service.getSession(sessionId)) return reply.code(404).send({ error: "Session not found" });
    return { sessionId, tree: service.getImportTree(sessionId), stats: service.getSessionStats(sessionId) };
  });

  server.post("/web/upload/tree/rename", async (request: FastifyRequest, reply: FastifyReply) => {
    const { sessionId, fileId, newName } = request.body as Record<string, string>;
    if (!sessionId || !fileId || !newName) return reply.code(400).send({ error: "sessionId, fileId, and newName are required" });
    try { await getStagingService().renameFile(sessionId, fileId, newName); return { success: true }; }
    catch (err) { return reply.code(400).send({ error: err instanceof Error ? err.message : String(err) }); }
  });

  server.post("/web/upload/tree/delete", async (request: FastifyRequest, reply: FastifyReply) => {
    const { sessionId, fileId } = request.body as Record<string, string>;
    if (!sessionId || !fileId) return reply.code(400).send({ error: "sessionId and fileId are required" });
    try { await getStagingService().removeFile(sessionId, fileId); return { success: true }; }
    catch (err) { return reply.code(400).send({ error: err instanceof Error ? err.message : String(err) }); }
  });

  server.post("/web/upload/tree/move", async (request: FastifyRequest, reply: FastifyReply) => {
    const { sessionId, fileId, newRelativePath } = request.body as Record<string, string>;
    if (!sessionId || !fileId || !newRelativePath) return reply.code(400).send({ error: "sessionId, fileId, and newRelativePath are required" });
    try { await getStagingService().moveFile(sessionId, fileId, newRelativePath); return { success: true }; }
    catch (err) { return reply.code(400).send({ error: err instanceof Error ? err.message : String(err) }); }
  });

  server.post("/web/upload/commit", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post("/web/upload/cancel", async (request: FastifyRequest, reply: FastifyReply) => {
    const { sessionId } = request.body as Record<string, string>;
    if (!sessionId) return reply.code(400).send({ error: "sessionId is required" });
  ...
  server.get("/web/upload/report/failed", async (request: FastifyRequest, reply: FastifyReply) => {
    const sessionId = (request.query as Record<string, string>).sessionId;
    if (!sessionId) return reply.code(400).send({ error: "sessionId is required" });
    const session = getStagingService().getSession(sessionId);
    if (!session) return reply.code(404).send({ error: "Session not found" });
    const lines = ["# ScrapeGoat — Files that failed to upload", `# Session: ${sessionId}`, `# Library: ${session.library} v${session.version}`, `# Generated: ${new Date().toISOString()}`, ""];
    ...
    reply.header("Content-Disposition", 'attachment; filename="Scrapegoat-FailedToUpload.txt"');
    return lines.join("\n");
  });

  server.get("/web/upload/report/renamed", async (request: FastifyRequest, reply: FastifyReply) => {
    const sessionId = (request.query as Record<string, string>).sessionId;
    if (!sessionId) return reply.code(400).send({ error: "sessionId is required" });
    const session = getStagingService().getSession(sessionId);
    if (!session) return reply.code(404).send({ error: "Session not found" });
    const lines = ["# ScrapeGoat — Files that were renamed during upload", `# Session: ${sessionId}`, `# Library: ${session.library} v${session.version}`, `# Generated: ${new Date().toISOString()}`, ""];
    for (const e of session.renamedFiles) lines.push(`${e.originalName}\t→\t${e.newName}\t(${e.reason})`);
    if (session.renamedFiles.length === 0) lines.push("# No files were renamed during upload.");
    reply.header("Content-Type", "text/plain; charset=utf-8");
    reply.header("Content-Disposition", 'attachment; filename="Scrapegoat-RenamedFiles.txt"');
    return lines.join("\n");
  });

  server.get("/web/upload/stats", async (request: FastifyRequest, reply: FastifyReply) => {
    const sessionId = (request.query as Record<string, string>).sessionId;
    if (!sessionId) return reply.code(400).send({ error: "sessionId is required" });
    const service = getStagingService();
    const session = service.getSession(sessionId);
    if (!session) return reply.code(404).send({ error: "Session not found" });
    return { sessionId, status: session.status, library: session.library, version: session.version, stats: service.getSessionStats(sessionId) };
  });
}

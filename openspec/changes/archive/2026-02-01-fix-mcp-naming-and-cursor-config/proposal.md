...
## Why
Consistency in naming the MCP server ("scrapegoat") avoids confusion. Cursor specifically works better with `streamableHttp` transport type, so documentation should reflect this best practice.

## What Changes
- Update `docs/guides/mcp-clients.md`:
  - Replace all occurrences of `"docs": {` with `"scrapegoat": {`.
  - Change Cursor configuration to use `"type": "streamableHttp"`.
- Update `docs/setup/installation.md`:
  - Replace `"docs": {` with `"scrapegoat": {` in the configuration example.
